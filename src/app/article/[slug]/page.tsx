...
import { Share2, Clock, Calendar, Facebook, Twitter, Linkedin, Copy, Printer, Bookmark, PlayCircle, Youtube } from "lucide-react";
import { getArticleBySlug, generateNewsList, getRelatedNews } from "@/data/mockNews";
...
// Note: Open Graph metadata should ideally be in a Server Component
// Since this is a client component, metadata will need to be added via layout or a server wrapper
// For now, keeping this client-side for compatibility with existing video player logic
...
const ArticlePage = () => {
    ...
    const { playVideo, closePlayer } = useVideoPlayer(); // Need closePlayer to stop floating if inline starts
    
    // Inline Player State
    const [isInlinePlaying, setIsInlinePlaying] = useState(false);
    const inlinePlayerRef = useRef<any>(null); // Ref to hold YT/FB player instance
    const cleanUpRef = useRef<boolean>(false); // Ref to track if we should handover
        ...
        return () => {
             ...
             if (isInlinePlaying && article?.relatedVideo) {
                 ...
                 playVideo({
                     ...
                     source: (article.relatedVideo.source as any), // Type cast
                     url: article.relatedVideo.id,
                     thumbnail: article.image,
                     startTime: startTime // PASS START TIME (Need to support this in Context)
                 });
             }
        };
    ...
    // Suggestion: Ensure getRelatedNews is imported
    const authorNews = getRelatedNews(article.title, article.catSlug || "latest", 4);
    ...
    return (
        ...
        <div className="min-h-screen bg-background text-foreground font-serif">
          ...
          <StructuredData 
            ...
            data={{
              ...
              image: article.image,
              datePublished: "2026-01-20T00:00:00.000Z", // Static date to avoid hydration mismatch
              author: {
                name: article.author,
              },
              publisher: {
          ...
          <main className="container mx-auto px-4 py-8 max-w-7xl">
            ...
            {/* Breadcrumb ... (Keep existing code) */}
            <div className="text-sm text-gray-500 mb-6 flex gap-2 items-center">
               <Link href="/">হোম</Link> 
               <span>/</span>
               <Link href={`/category/${article.catSlug}`} className="text-brand-red font-semibold hover:underline">
                  {article.category}
               </Link>
            </div>
            ...
            {/* Author & Meta ... (Keep existing code) */}
               ...
               <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                     ...
                     <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs md:text-sm text-gray-500 mt-1">
                        ...
                        <span className="flex items-center gap-1.5"><Clock size={14} /> {localizeTime(article.time)}</span>
                     </div>
                  </div>
               </div>
    
               {/* Share Button Group (Keep logic) */}
            ...
            <div className="relative aspect-video w-full md:w-[85%] rounded-xl overflow-hidden mb-8 shadow-sm">
               ...
               <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6 pt-12">
                  <p className="text-white text-sm font-medium opacity-90">ছবি: প্রতীকী চিত্র (আনস্প্ল্যাশ)</p>
               </div>
            </div>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
           <div className="flex flex-wrap gap-2">
              <span className="bg-gray-100 px-4 py-1.5 rounded-full text-sm font-medium text-gray-600 hover:bg-gray-200 hover:text-brand-red transition cursor-pointer">#বাংলাদেশ</span>
              <span className="bg-gray-100 px-4 py-1.5 rounded-full text-sm font-medium text-gray-600 hover:bg-gray-200 hover:text-brand-red transition cursor-pointer">#{article.category}</span>
              <span className="bg-gray-100 px-4 py-1.5 rounded-full text-sm font-medium text-gray-600 hover:bg-gray-200 hover:text-brand-red transition cursor-pointer">#সংবাদ</span>
           </div>
        </div>
        ...
        <div className="mt-16 pt-12 border-t-4 border-gray-900">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {getRelatedNews(article.title, article.catSlug || "latest", 8).map((news) => (
                    <Link key={news.id} href={`/article/${news.id}`} className="group block bg-white rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 border border-gray-100 hover:-translate-y-1">
                        ...
                        <div className="p-4">
                            ...
                            <div className="flex items-center gap-2 text-xs text-gray-400 mt-2">
                                <Clock size={12} />
                                <span>{localizeTime(news.time)}</span>
                            </div>
                        </div>
                    </Link>
                ))}
            ...
            <div className="lg:col-span-3">
                <aside className="sticky bottom-4">
                    <LatestSidebarWidget 
                        news={generateNewsList("politics", 10)} // Using politics/latest mix for demo
                    />
